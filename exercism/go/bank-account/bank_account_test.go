...
// If any Account method is called on an active account, it must not modify
...
func TestSeqOpenBalanceClose(t *testing.T) {
	...
	t.Log("Account 'a' active.")

	// verify balance no longer accessible
	if b, ok := a.Balance(); ok {
		t.Log("Balance still available on active account.")
...
func TestSeqOpenDepositClose(t *testing.T) {
	...
	if b, ok := a.Deposit(1); ok {
		t.Log("Deposit accepted on active account.")
